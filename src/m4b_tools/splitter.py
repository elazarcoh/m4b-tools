...
def extract_chapters_from_m4b(
    file_path: str,
) -> Tuple[List[ChapterInfo], AudioMetadata]:
    """
    ...
    try:
        ...
        for i, chapter in enumerate(data.get("chapters", [])):
            ...
            chapters.append(
                ChapterInfo(title=title, start=start_time, end=end_time, index=i + 1)
            )
...
def split_m4b_file(
    ...
    if f".{output_format}" not in SUPPORTED_SPLIT_FORMATS:
        ...
        logger.info(
            f"Supported formats: {', '.join(f[1:] for f in SUPPORTED_SPLIT_FORMATS)}"
        )
        return 0, 0
...
def split_multiple_m4b_files(
    ...
    for file_path in m4b_files:
        ...
        succ, total = split_m4b_file(
            file_path, output_dir, output_format, template, max_workers
        )
    ...
    logger.info(
        f"Split complete: {successful_files}/{len(m4b_files)} files, "
