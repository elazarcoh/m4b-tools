...
def convert_to_m4b(
    input_file: str, output_file: str, codec: str = "aac", bitrate: str = "64k"
) -> bool:
    """
    ...
    Args:
        ...
        codec: Audio codec to use (aac, aac_he, alac)
    ...
    try:
        ...
        codec_params = get_codec_parameters(codec, bitrate)
        ...
        logger.info(
            f"Converting: {os.path.basename(input_file)} -> {os.path.basename(output_file)} (codec: {codec}, bitrate: {bitrate})"
        )
...
def _process_single_file(
    ...
    if preserve_structure:
        ...
        if base_input_path and base_path:
            ...
            try:
                ...
                logger.warning(
                    f"File {input_file} is not under base_input_path {base_input_path}, using filename only"
                )
    ...
    # Convert the file
    success = convert_to_m4b(str(input_path), str(output_path), codec, bitrate)
    return success, input_file


def convert_all_to_m4b(
    glob_pattern: str,
    output_base_dir: str,
    preserve_structure: bool = True,
    show_progress_bar: bool = False,
    base_input_path: Optional[str] = None,
    max_workers: int = 1,
    codec: str = "aac",
    bitrate: str = "64k",
) -> Tuple[int, int]:
    ...
    if base_input_path:
        ...
        if not os.path.isabs(glob_pattern):
            ...
            logger.warning(
                "Glob pattern is absolute, base_input_path will be used for structure only"
            )
    else:
        full_pattern = glob_pattern
        base_path = None
    ...
    audio_files = [
        f for f in matching_files if Path(f).suffix.lower() in SUPPORTED_FORMATS
    ]

    if not audio_files:
        logger.warning(
            f"No supported audio files found matching pattern: {glob_pattern}"
        )
        return 0, 0
    ...
    logger.info(
        f"Starting conversion of {total_files} files with {max_workers} worker(s)..."
    )
