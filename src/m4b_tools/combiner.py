...
def create_chapter_metadata(
    chapters: List[Chapter], temp_dir: str, metadata: BookMetadata
) -> str:
...
def check_audio_compatibility(files_metadata: List[ProcessedAudioMetadata]) -> bool:
    ...
    for metadata in files_metadata[1:]:
        if (
            metadata.get("codec") != reference_codec
            or metadata.get("sample_rate") != reference_sample_rate
            or metadata.get("channels") != reference_channels
        ):
            return False
...
def derive_chapter_title(file_path: str, index: int, existing_title: str = "") -> str:
    ...
    cleaned = re.sub(
        r"^(chapter|ch|part|pt)[\s\-_]*\d*[\s\-_]*", "", filename, flags=re.IGNORECASE
    )
...
def generate_csv_from_folder(
    folder_path: str, output_csv: Optional[str] = None
) -> bool:
    """
...
def generate_csv_from_multiple_folders(
    folder_pattern: str, base_output_csv: Optional[str] = None
) -> bool:
    """
    ...
    logger.info(
        f"Found {len(matching_folders)} directories matching pattern: {folder_pattern}"
    )
...
def generate_csv_from_single_folder(
    folder_path: str, output_csv: Optional[str] = None
) -> bool:
    """
    ...
    for file_path in glob.glob(os.path.join(folder_path, "**", "*"), recursive=True):
        if os.path.isfile(file_path) and Path(file_path).suffix.lower() in {
            ".m4b",
            ".m4a",
        }:
    ...
    def get_most_common_or_first(field):
        values = [
            meta.get(field, "").strip()
            for meta in all_metadata
            if meta.get(field, "").strip()
        ]
        if not values:
            return ""
    ...
    template_title = (
        get_most_common_or_first("album")
        or get_most_common_or_first("title")
        or folder_name
    )
    template_author = get_most_common_or_first("author") or get_most_common_or_first(
        "artist"
    )
    ...
    logger.info(
        f"Detected metadata - Title: '{template_title}', Author: '{template_author}', Narrator: '{template_narrator}', Year: '{template_year}'"
    )
        ...
        with open(output_csv, "w", encoding="utf-8", newline="") as f:
            ...
            for file_path in m4b_files:
                ...
                if metadata_title:
                    ...
                    cleaned = re.sub(
                        r"^(chapter|ch|part|pt)[\s\-_]*\d*[\s\-_]*",
                        "",
                        filename,
                        flags=re.IGNORECASE,
                    )
...
def parse_csv_input(csv_file: str) -> Tuple[List[FileEntry], Dict[str, str]]:
    ...
    for row in csv_reader:
        ...
        file_entry: FileEntry = {"file": file_path, "title": title}
    ...
    logger.info(
        f"Loaded {len(file_list)} files from CSV with {len(metadata)} metadata entries"
    )

    return file_list, metadata


def combine_m4b_files(
    ...
    Args:
        ...
        codec: Audio codec to use (aac, aac_he, alac). Can be overridden by CSV metadata
        bitrate: Audio bitrate (e.g., '64k', '128k'). Can be overridden by CSV metadata
    ...
    if csv_file:
        ...
        m4b_files = [
            ...
            if Path(f).suffix.lower() in {".m4b", ".m4a"}
        ]
    ...
    # Check audio compatibility and get codec/bitrate from CSV if provided
    codec_to_use = csv_metadata.get("codec", codec)
    bitrate_to_use = csv_metadata.get("bitrate", bitrate)
    ...
    if compatible:
        ...
        logger.info(
            f"Files have different audio parameters - re-encoding with codec: {codec_to_use}, bitrate: {bitrate_to_use}"
        )
        encode_params = get_codec_parameters(codec_to_use, bitrate_to_use)
    ...
    for i, file_meta in enumerate(files_metadata, 1):
        ...
        if preserve_existing_chapters:
            ...
            if existing_chapters:
                logger.info(
                    f"  File {i} has {len(existing_chapters)} existing chapters"
                )
                for chapter in existing_chapters:
                    base_title = csv_title or derive_chapter_title(
                        file_meta["file_path"], i, file_meta.get("title", "")
                    )
                    chapters.append(
                        {
                ...
                chapter_title = csv_title or derive_chapter_title(
                    file_meta["file_path"], i, file_meta.get("title", "")
                )
                chapters.append(
                    {
    ...
    logger.info(
        f"Created {len(chapters)} chapters, total duration: {format_time(total_duration)}"
    )
    ...
    with temp_context as use_temp_dir:
        ...
        output_metadata: BookMetadata = {
            "title": title
            or csv_metadata.get("title")
            or files_metadata[0].get("album", "")
            or "Combined Audiobook",
        ...
        cmd = ["ffmpeg", "-i", temp_combined, "-i", metadata_file]
