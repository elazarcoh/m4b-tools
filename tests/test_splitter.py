...
class TestSplitterModule:
    ...
    def test_sanitize_filename(self):
        ...
        assert (
            sanitize_filename("name<>:with|invalid*chars") == "name_with_invalid_chars"
        )
        assert (
            sanitize_filename('name"with/quotes\\and\\slashes')
            == "name_with_quotes_and_slashes"
        )

        # Test multiple consecutive underscores
        assert (
            sanitize_filename("name___with___many___underscores")
    ...
    def test_format_chapter_filename(self):
        ...
        assert result.startswith(
            "John Doe/My Great Book/Chapter 01 - The Beginning [2m 0s].mp3"
        )

        # Test template with missing metadata
        empty_metadata: AudioMetadata = {}
        result = format_chapter_filename(
            "{author}/{book_title}.{ext}",
            chapter,
            empty_metadata,
            "mp3",
            "/path/to/book.m4b",
    ...
    def test_extract_chapters_from_m4b_with_chapters(
        self, mock_get_metadata, mock_subprocess
    ):
    ...
    @patch("m4b_tools.splitter.subprocess.run")
    @patch("m4b_tools.splitter.get_audio_metadata")
    def test_extract_chapters_from_m4b_no_chapters(
        self, mock_get_metadata, mock_subprocess
    ):
