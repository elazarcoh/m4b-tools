...
class TestAudioFunctionality:
    ...
    def test_convert_single_flac_to_m4b(self):
        ...
        flac_file = self.create_test_audio_file(
            "test_audio", duration=2.5, format_name="flac"
        )
    ...
    def test_convert_all_multiple_files(self):
        ...
        files_created.append(
            self.create_test_audio_file("file1", duration=2.0, format_name="mp3")
        )
        files_created.append(
            self.create_test_audio_file("file2", duration=1.5, format_name="flac")
        )
        files_created.append(
            self.create_test_audio_file("file3", duration=2.5, format_name="m4a")
        )
        ...
        successful, total = convert_all_to_m4b(
            pattern, output_dir, preserve_structure=False
        )
    ...
    def test_convert_all_with_preserve_structure(self):
        ...
        file1 = self.create_test_audio_file(
            "root_file", duration=2.0, format_name="mp3"
        )
        ...
        cmd = [
            ...
            "lavfi",
            "-i",
            "sine=frequency=220:duration=1.5:sample_rate=22050",
            "-ac",
            "1",
            "-y",
            sub_file_path,
        ...
        successful, total = convert_all_to_m4b(
            pattern, output_dir, preserve_structure=True
        )
    ...
    def test_combine_m4b_files(self):
        ...
        for i, duration in enumerate(durations, 1):
            ...
            source_file = self.create_test_audio_file(
                f"source{i}", duration=duration, format_name="mp3"
            )
        ...
        result = combine_m4b_files(
            input_pattern=pattern, output_file=combined_output, title="Test Audiobook"
        )
        ...
        metadata = self.verify_audio_file(
            combined_output, expected_duration=expected_total_duration, tolerance=1.0
        )
    ...
    def test_combine_with_csv_file(self):
        ...
        for i in range(1, 4):
            source_file = self.create_test_audio_file(
                f"chapter{i}", duration=2.0, format_name="mp3"
            )
            m4b_file = os.path.join(self.temp_dir, f"chapter{i}.m4b")
            result = convert_to_m4b(source_file, m4b_file)
    ...
    def test_csv_preserves_order(self):
        ...
        for name in file_names:
            source_file = self.create_test_audio_file(
                name.replace(".m4b", ""), duration=1.0, format_name="mp3"
            )
        ...
        csv_order = [
            ...
            "chapter1.m4b",
            "chapter10.m4b",
        ]  # Intentionally out of natural order
    ...
    def test_generate_csv_from_folder(self):
        ...
        for i in range(1, 4):
            source_file = self.create_test_audio_file(
                f"part{i:02d}", duration=1.5, format_name="mp3"
            )
    ...
    def test_generate_csv_from_multiple_folders_glob_pattern(self):
        ...
        for folder_name in test_folders:
            ...
            for i in range(1, num_files + 1):
                source_file = self.create_test_audio_file(
                    f"{folder_name}_part{i:02d}", duration=1.5, format_name="mp3"
                )
        ...
        for folder_name in test_folders:
            if folder_name.startswith("book"):
                csv_file = os.path.join(
                    self.temp_dir, folder_name, f"{folder_name}.csv"
                )
                assert os.path.exists(csv_file), (
                    f"CSV file should exist for {folder_name}"
                )
    ...
    def test_generate_csv_from_multiple_folders_recursive_pattern(self):
        ...
        for series in base_folders:
            ...
            for book_num in range(1, 3):
                ...
                for i in range(1, 3):
                    source_file = self.create_test_audio_file(
                        f"{series}_{book_folder}_part{i:02d}",
                        duration=1.5,
                        format_name="mp3",
                    )
        ...
        for series in base_folders:
            for book_num in range(1, 3):
                book_folder = f"book{book_num}"
                csv_file = os.path.join(
                    self.temp_dir, series, book_folder, f"{book_folder}.csv"
                )
                assert os.path.exists(csv_file), (
                    f"CSV file should exist for {series}/{book_folder}"
                )
    ...
    def test_generate_csv_multiple_folders_empty_directories(self):
        ...
        assert result is False, (
            "CSV generation should fail when directories have no M4B files"
        )
    ...
    def test_generate_csv_mixed_folders_some_with_files(self):
        ...
        for i in range(1, 3):
            source_file = self.create_test_audio_file(
                f"good_part{i:02d}", duration=1.5, format_name="mp3"
            )
        ...
        result = generate_csv_from_folder(pattern)
        assert result is True, (
            "CSV generation should succeed if at least one folder has files"
        )

        # Verify CSV was created for the good folder
        csv_file = os.path.join(good_folder, "good_book.csv")
        assert os.path.exists(csv_file), (
            "CSV file should exist for folder with M4B files"
        )

        # Verify no CSV was created for the empty folder
        empty_csv_file = os.path.join(empty_folder, "empty_book.csv")
        assert not os.path.exists(empty_csv_file), (
            "CSV file should not exist for empty folder"
        )
    ...
    def test_generate_csv_single_folder_vs_glob_behavior(self):
        ...
        for i in range(1, 4):
            source_file = self.create_test_audio_file(
                f"single_part{i:02d}", duration=1.5, format_name="mp3"
            )
    ...
    def test_generate_csv_with_custom_output_ignored_for_glob(self):
        ...
        assert not os.path.exists(custom_output), (
            "Custom output should be ignored for glob patterns"
        )
    ...
    def test_generate_csv_folder_with_subdirectories(self):
        ...
        for i in range(1, 3):
            source_file = self.create_test_audio_file(
                f"root_part{i:02d}", duration=1.5, format_name="mp3"
            )
        ...
        for i in range(1, 3):
            source_file = self.create_test_audio_file(
                f"bonus_part{i:02d}", duration=1.5, format_name="mp3"
            )
